...
\begin{minipage}[c]{10cm}
  \question[19] Se dispone de un sistema formado por 3 cargas puntuales: $q_1=8[nC]$, $q_2=-25[nC]$ y $q_3=24[nC]$ ubicadas en los puntos $P_1(4,8,0)$, $P_2(0,4,3)$ y $P_3(4,4,2)$ respectivamente. Considere que las posiciones de las cargas están en centímetros y que el valor de $K=9\times10^9[Nm^2/C^2]$, responda\\
  Nota:\\
 ...
 \draw[step=1cm,very thin, gray!40] (-3,-2) grid (4,6);
    %  \draw[line width=1.4,dashed,gray] (0,0) circle (3);
      \draw[black, line width =1.3,->, >=latex] (0,0) -- (0,6) node [black, right, scale=1.5] {$Y$};
       \draw[black, line width =1.3,->, >=latex] (0,0) -- (4,0) node [black, above, scale=1.5] {$X$};
       \draw[black,line width=1.3, ->, >=latex] (0,0) -- (-2.5,-2) node [black, above,scale=1.5] at (-2.5,-1.8) {$Z$};
      \node[black, right, scale=1.3,right] at (0,4.3) {$P$};
     %\fill (-2.55,-1.7) circle (0.2);
       \node[black, scale=1.3] at (-1.4,-1.5) {$3$};
       % \fill (0,-3) circle (0.2);
 \node[black, scale=1.3,below] at (2,0) {$4$};
     \fill (2,4) circle (0.2);
    \draw[black, dashed] (2,0) -- (2,4) node[black, scale=1.3, xshift=15pt] at (2,4) {$q_1$};
   \draw[black, dashed] (0,4) -- (2,4) node[black, scale=1.3, xshift=-10pt] at (0,4) {$8$}; 
\fill (0,4) circle (0.2);
  \draw[black, dashed] (-1.3,-1) -- (-1.3,1) node[black, right, scale=1.3] at (-1.9,1.5) {$q_2$}; 
  \draw[black, dashed] (-0.5,1.5) -- (1.5,1.5);
\draw[black, dashed] (0,2) -- (-1.3,1) node[black, right, scale=1.3] at (0,2) {$4$};
\draw[black, dashed] (0,2) -- (2,2); %node[black, right, scale=1] at (1.5,2) {$q_3$};
\draw[black, dashed] (1.5,1.5) -- (2,2);
\draw[black, dashed] (1.5,-0.5) -- (-0.5,-0.5) node[black, right, scale=1.3] at (1.5,1) {$q_3$};
...
\end{minipage}
    \begin{parts}
        \part {\bf\red{(8.0 pts)}} calcule el potencial eléctrico en un punto P(0,8,0)
        \vspace{8cm}
        \part {\bf\red{(8.0 pts)}} calcule la energía contenida en el sistema.
        \vspace{8cm}
        \part {\bf\red{(3.0 pts)}} calcule el trabajo necesario en traer una carga $q_4=-7[nC]$ al punto P.
        \vspace{4cm}
        \end{parts}
...
\newpage
%%%%%%%%%%%%%%%%%    PAUTA PROBLEMA 3 %%%%%%%%
\includepdf[pages=1-1]{Pauta-P2_A__C2_ELECTRO_UBB_S2_2026.pdf}
